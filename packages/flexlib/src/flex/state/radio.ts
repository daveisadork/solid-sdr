...
export interface RadioSnapshot {
  ...
  /** Display profiles are absent on 4.x firmware — never observed on the wire. */
  ...
  readonly oscillatorState: RadioOscillatorSetting;
  ...
  for (const [key, value] of Object.entries(attributes)) {
    switch (key) {
      case "state": {
        ...
        if (parsed) partial.oscillatorState = parsed;
        else if (value) logParseError("radio", "oscillator.state", value);
        break;
      }
      case "setting": {
        const parsed = parseOscillatorSetting(value);
